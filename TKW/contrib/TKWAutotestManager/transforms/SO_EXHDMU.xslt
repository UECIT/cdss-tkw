<?xml version="1.0"?>
<!-- $Id: SO_EXHDMU.xslt 557 2016-03-07 13:58:11Z sfarrow $ -->
<!-- SO_EXHDMU local header with mustUnderstand = 0 -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:output method="xml" indent="yes" omit-xml-declaration="yes" />
    <!-- match when there is already a local header elelemt -->
    <xsl:template match="local:LocalHeaderElement" xmlns:local="local-namespace-uri">
        <xsl:comment>mustUnderstand attribute set to 0 by TKW AutotestManager</xsl:comment>
    ...
    <xsl:template match="soap:Header/*[last()]" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:local="local-namespace-uri">
        ...
        <xsl:if test="not(../local:LocalHeaderElement)">
            <xsl:comment>LocalHeaderElement with mustUnderstand attribute set to 0 added by TKW AutotestManager</xsl:comment>
            <xsl:element name="local:LocalHeaderElement">
